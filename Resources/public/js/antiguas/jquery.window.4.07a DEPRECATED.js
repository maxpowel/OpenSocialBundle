...
$.Window = (function()  {
	...
	// static constants
	var VERSION = "4.07a";           // the version of current plugin
	...
	function constructor(caller, options) {
		...
		function initialize(instance) {
			...
			realCaller.append("<div id='"+windowId+"' class='window_panel "+cornerClass+options.containerClass+" portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all'></div>");
			...
			var headerHtml = "<div class='window_header window_header_normal portlet-header ui-widget-header ui-corner-all "+options.headerClass+"'>"+
			...
			// add close button
			if( options.closable ) {
				headerFuncPanel.append( "<div title='close window' class='cerrar ui-icon ui-icon-closethick window_icon_button no-draggable'></div>" );
				headerFuncPanel.children(".cerrar").click(function() {
					close();
				});
			...
			if( options.maximizable ) {
				...
				headerFuncPanel.append( "<div title='maximize window' class='maximizeImg ui-icon ui-icon-arrow-4-diag window_icon_button no-draggable'></div>" );
				headerFuncPanel.append( "<div title='cascade window' class='cascadeImg ui-icon ui-icon-newwin window_icon_button no-draggable' style='display:none;'></div>" );
			...
			if( options.minimizable ) {
				headerFuncPanel.append( "<div title='minimize window' class='minimizar ui-icon ui-icon-minusthick window_icon_button no-draggable'></div>" );
				headerFuncPanel.children(".minimizar").click(function() {
					//minimizar($(this).parent().parent().parent());
					minimize();
				});
				funcBarWidth += ICON_OFFSET;
			}
			...
			if( options.showFooter ) {
				...
				container.append("<div class='window_footer portlet-content no-draggable "+options.footerClass+"'><div></div></div>");
			...
			if( options.resizable ) {
				container.resizable({
					...
					stop: function() {
						...
						showContent();
						// callback
						if( $.isFunction(options.afterResize) ) {
							options.afterResize(me);
						}
					}
				});
				// set boundary if got opotions
		...
		function restore() {
			...
			container.animate(targetCssStyle, animationSpeed, 'swing', function() {
				...
				if( $.isFunction(options.afterCascade) ) {
					options.afterCascade(me);
				}
			});
			
			// before callback
			if( $.isFunction(options.onCascade) ) {
				options.onCascade(me);
			}
