...
$.fn.photoUpload = function () {
	...
	$(this).find("button").button().click(function(){
                ...
                return;
		state.val(1);
                console.log(document.getElementById("lefile"));
                sendMultipleFiles({
                    //url:"index.php",
                    // list of files to upload
                    //files:input.files,
                    files:[document.getElementById("lefile")],
                    //files:container.find(".file"),
                    ...
                    onprogress:function(rpe){
                        ...
                        console.log("Uploading: " + this.file.fileName);
                        console.log("Sent: " + size(rpe.loaded) + " of " + size(rpe.total));
                        console.log("Total Sent: " + size(this.sent + rpe.loaded) + " of " + size(this.total));
	...
	form = $("<div style='padding:3px'>"+
			'<form enctype="multipart/form-data" action="app_dev.php/upload" method="POST">'+
				...
				"<input  id='lefile' type='file' name='file' class='file' style='float:right; position:absolute; z-index: 1; opacity:0'>" +
